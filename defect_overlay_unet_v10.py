# ===================== defect_overlay_unet_v10.py =====================
"""
CPU-only U-Net that learns to paint defect overlays on wafer cross-sections.

Changes from v9
---------------
• 4-level U-Net (32-64-128-256-128-64-32)  ≈ 300 k params  
• Adds SSIM term (weight 0.05) to the loss

Everything else—including the fixed‐threshold masking pipeline, folder
structure, preview grid, and one-off defect-map montage—remains identical.
"""
# ---------------------------------------------------------------------
#                               Imports
# ---------------------------------------------------------------------
...
OUT_DIR     = "outputs/defect_overlay_cpu_v10"
CKPT_DIR    = f"{OUT_DIR}/ckpt_v10"
...
class OverlayDS(Dataset):
    """(good_bg, defect_overlay, mask) for each *defect* image."""
    def __init__(self):
...
def sample_gan(n):
    ...
    full = Gbg(z)
    small = F.interpolate(full,(RES_TRAIN,RES_TRAIN),
                          mode='bilinear',align_corners=False) \
            if RES_TRAIN!=RES_FULL else full
    return full, small

# ---------------------------------------------------------------------
#                  4-level U-Net (32 base channels)
# ---------------------------------------------------------------------
...
class MidUNet(nn.Module):
    def __init__(self,ch=32):
        super().__init__()
        self.e1=down(1,ch); self.e2=down(ch,ch*2)
        self.e3=down(ch*2,ch*4); self.e4=down(ch*4,ch*8)
        self.d4=up(ch*8,ch*4)
...
def main():
    ...
    for ep in range(1,EPOCHS+1):
        for bg,ov_gt,mask_gt in tqdm(loader,desc=f"Epoch {ep}/{EPOCHS}",leave=False):
            ...
            loss=( L1(ov_pred,ov_gt)+
                   ...
                   SSIM_W  *(1-ssim((ov_pred+1)/2,(ov_gt+1)/2,data_range=1)) )
        ...
        with torch.no_grad():
            ...
            rows=[ torch.clamp(real_bg_full+real_ov_full,-1,1),        # defect img
                   torch.clamp(white+real_ov_full,-1,1),               # GT overlay
                   torch.clamp(white+ov_pred_full,-1,1),               # pred overlay
                   synth_full ]                                        # pred on GAN bg
        ...
        print(f"Epoch {ep} finished – loss {loss.item():.4f}")
