# ===================== defect_overlay_unet_v8.py ======================
"""
CPU-only overlay trainer – defects-only version.

Key differences from v3/v7
--------------------------
1. Dataset contains **only true defect images** (≈50) each paired with its
   nearest good background.
2. Overlay creation reverts to fixed absolute threshold (THRESH_ABS = 0.12)
   so every sample carries a visible defect map.
3. All earlier bug-fixes (quick_vec NumPy, Gaussian blur channel fix, Dice
   regulariser, PyTorch morphology, etc.) are kept.

Outputs land in  outputs/defect_overlay_cpu_v8/  mirroring the prior layout.
"""
...
THRESH_ABS  = 0.12          # fixed threshold on |defect − good| in [-1,1]
SPARSE_W    = 5e-3
EDGE_W      = 1.0
DICE_W      = 0.2
TV_W        = 0.0
...
OUT_DIR     = "outputs/defect_overlay_cpu_v8"
CKPT_DIR    = f"{OUT_DIR}/ckpt_v8"
...
def _gauss_kernel(ch=1, k=5, σ=1.0):
    ...
    ker = g2d[None,None].repeat(ch,1,1,1)
    return ker
