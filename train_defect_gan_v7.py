...
# ====================
def denorm(x):
    return (x + 1.0) / 2.0  # from [-1,1] to [0,1]

def save_visual(good, generated, epoch, out_dir, tag="recon"):
    img = torch.cat([good, generated], dim=0)
    utils.save_image(denorm(img), os.path.join(out_dir, f"{tag}_epoch_{epoch:02d}.png"), nrow=4)
...
def train():
    ...
    out_dir = "./outputs/fake_defects_v7"
    ckpt_dir = "./outputs/checkpoints_v7"
    ...
    # =====================
    print("🔧 Pretraining Generator on identity (good → good)")
    ...
    for epoch in range(1, 11):
        ...
        for good, _ in pbar:
            ...
            loss.backward()
            opt_pre.step()
            pbar.set_postfix(pretrain_loss=loss.item())

        with torch.no_grad():
            save_visual(good[:4], G(good[:4]), epoch, out_dir, tag="pretrain")

    torch.save(G.state_dict(), os.path.join(ckpt_dir, "pretrained_generator_v7.pt"))
    ...
    for epoch in range(1, 51):
        ...
        for good, defect in pbar:
            ...
            loss_D_fake = adversarial_loss(pred_fake_detach, fake)
    ...
    plt.title("Loss Curve (v7)")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.savefig(os.path.join(out_dir, "loss_curve_v7.png"))
    plt.close()
