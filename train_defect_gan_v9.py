...
class UNetGenerator(nn.Module):
    ...
    def forward(self, x):
        ...
        m = self.middle(d4)       # (B, 512, 8, 8)

        def match_size(x, target):
            return F.interpolate(x, size=target.shape[2:], mode="bilinear", align_corners=False)

        u4 = self.up4(torch.cat([match_size(m, d4), d4], dim=1))  # → (B, 256, 16, 16)
        u3 = self.up3(torch.cat([match_size(u4, d3), d3], dim=1)) # → (B, 128, 32, 32)
        u2 = self.up2(torch.cat([match_size(u3, d2), d2], dim=1)) # → (B, 64, 64, 64)
        out = self.up1(torch.cat([match_size(u2, d1), d1], dim=1)) # → (B, 1, 128, 128 or 256x256 depending)
...
def train():
    ...
    out_dir = "./outputs/fake_defects_v9"
    ckpt_dir = "./outputs/checkpoints_v9"
    ...
    for epoch in range(1, 11):
        pbar = tqdm(loader, desc=f"[Pretrain {epoch}/10]")
        for good, _ in pbar:
            good = good.to(device)
            recon = G(good)
            loss = pixel_loss(recon, good) + 0.5 * edge_loss(sobel(recon), sobel(good))
    ...
    torch.save(G.state_dict(), os.path.join(ckpt_dir, "pretrained_generator_v9.pt"))
    ...
    for epoch in range(1, 51):
        ...
        for good, defect in pbar:
            ...
            loss_G_edge = edge_loss(sobel(fake_defect), sobel(defect))
            loss_G = loss_G_adv + 1.0 * loss_G_l1 + 0.5 * loss_G_edge
            loss_G.backward()
            opt_G.step()
            ...
            loss_D_fake = adv_loss(D(good, fake_defect.detach()), fake)
    ...
    plt.title("Loss Curve (v9)")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.savefig(os.path.join(out_dir, "loss_curve_v9.png"))
    plt.close()
